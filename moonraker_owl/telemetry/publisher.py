"""TelemetryPublisher �?consumes Moonraker updates and forwards
telemetry envelopes to MQTT.

Extracted from ``telemetry/__init__.py`` to reduce module size.
The package ``__init__`` re-exports all public symbols from here.
"""
...
class TelemetryPublisher:
    ...
    async def start(self, *, preserve_print_state: bool = False) -> None:
        ...
        try:
            await self._source.start(self._handle_moonraker_update)
    ...
    async def request_print_state_query(self) -> None:
        ...
        try:
            ...
            snapshot = await self._source.fetch_printer_state(
    ...
    async def _dispose_worker(self, *, remove_callback: bool = False) -> None:
        ...
        if remove_callback and self._callback_registered:
            self._source.remove_callback(self._handle_moonraker_update)
            self._callback_registered = False
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if not baseline_captured:
                    try:
                        files = await self._source.list_timelapse_files()
    ...
    async def _poll_loop(self, group: _PollGroup) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                payload = await self._source.fetch_printer_state(group.objects)
            except asyncio.CancelledError:
                raise
            except Exception as exc:  # pragma: no cover - defensive logging
        ...
        for event in events:
            ...
            try:
                metadata = await asyncio.wait_for(
                    self._source.fetch_gcode_metadata(filename),
    ...
    async def _fetch_and_set_moonraker_job_id(
        ...
        try:
            job = await asyncio.wait_for(
                self._source.fetch_most_recent_job(),
                timeout=timeout_seconds,
            )
    ...
    def _schedule_resubscribe(self, reason: str) -> None:
        ...
        async def _runner() -> None:
            ...
            try:
                ...
                LOGGER.warning(
                    "Failed to re-discover sensors after %s: %s",
                    reason,
                    exc,
                )
            try:
